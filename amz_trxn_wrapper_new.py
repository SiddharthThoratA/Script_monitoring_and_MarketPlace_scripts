...
#tgt_s3="s3://desototech/DWH_team/Ashwini/Marketplace/Amazon/Transactions"
#trigger_s3="s3://desototech/DWH_team/Ashwini/Marketplace/Amazon/Transactions/trigger_files"
#run_date = '20240808'#script running on this date
#prev_day = '20240806' # script will extract this day's data (RUN_DATE-2)
#prev_date_f = '2024-08-07' # yesterday date in YYYY-MM-DD format (created since)
#run_date_f = '2024-08-08'  # script run_date in YYYY-MM-DD format  (created until)
...
def send_start_end_mail(subject,body):
    log_file_email = f"{log_dir}\\email_notification_{run_date}.log"
    config_file = "email_config_dev.config"
...
def file_modification(file_type):
    ...
    with open(log_extr, "w") as f:
        subprocess.run(["python",f"amz_transaction_modification_dev.py", "-d", prev_day, "-c", f"amz_transaction_redshift_dev.config","-t", file_type], stdout=f,stderr=f)
        time.sleep(2)
